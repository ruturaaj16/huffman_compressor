...
## 🚀 Performance & Impact
**Space Efficiency:** Achieved a measurable space-saving of **30-50%** on standard text datasets.
**Algorithmic Complexity:** Optimized for **$O(N \log K)$** time complexity using a Min-Heap (Priority Queue)[cite: 16].
**Data Integrity:** Guaranteed 100% reconstruction of original data through bi-directional processing (encoding and decoding).
...
2. **Greedy Tree Construction:** Utilizes a **Min-Heap** to repeatedly merge the two least frequent nodes until a single **Huffman Tree** root remains.
3. **Prefix-Free Encoding:** Traverses the tree to assign unique binary codes (0 for left, 1 for right), ensuring no code is a prefix of another.
